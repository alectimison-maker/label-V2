...
```text
class x1 y1 v1 x2 y2 v2 ... x32 y32 v32
```

示例：

```text
0 0.329087 0.247311 2 0.361214 0.249802 2 ... 0.000000 0.000000 0
```

说明：

- `class` 固定为 `0`
- 后面的关键点按模板顺序导出，共 **32 个点**（4 个手工角点 + 28 个自动投影点），坐标都已归一化到 `0 ~ 1`
- 每个点统一导出 `x y visibility`：包括 `top_left`、`top_right`、`bottom_left`、`bottom_right` 四个手工角点在内，所有点均与后续自动点采用同样的 `x y visibility` 格式
- `visibility` 保持 `0 / 1 / 2`
